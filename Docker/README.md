...
`docker pull ummidock/innuca:4.0.1-05`
...
2) `docker build -t ummidock/innuca:4.0.1-05 ./INNUca/Docker/`

### Run (using automated build image)
    docker run --rm -u $(id -u):$(id -g) -it -v /local/folder/fastq_data:/data/ ummidock/innuca:4.0.1-05 INNUca.py --speciesExpected "Streptococcus agalactiae" --genomeSizeExpectedMb 2.1 --inputDirectory /data/ --outdir /data/innuca_output/ --threads 8 --maxNumberContigs 100
...
```bash
# Get Docker image
udocker pull ummidock/innuca:4.0.1-05

# Create container (only needed to be done once)
udocker create --name=innuca_4-0-1_05 ummidock/innuca:4.0.1-05

# Run INNUca
udocker run --user $(id -u):$(id -g) -v /local/folder/fastq_data:/data/ innuca_4-0-1_05 INNUca.py --speciesExpected "Streptococcus agalactiae" --genomeSizeExpectedMb 2.1 --inputDirectory /data/ --outdir /data/innuca_output/ --threads 8 --maxNumberContigs 100
```
